...
- Owning Pi `AgentSessionRuntime` so Extension commands receive a TUI-capable mode, official command-context actions, Session replacement/rebinding, diagnostics, async errors, shutdown requests, and cancellable/timeout-bound Extension UI requests; components still render in PiHost rather than entering the Renderer.
...
- Reading Pi's effective keybindings and invoking Pi's configured external-editor helper for the Renderer input adaptation; the Pi settings surface reads the effective command/source and writes the user command through Pi's locked `FileSettingsStorage`. Pi splits editor commands on spaces before spawning, so on macOS/Linux PiHost temporarily replaces quoted selected absolute paths with no-space symlink aliases, calls the same Pi helper, then removes the aliases. The Renderer never spawns editors or reads Pi config files directly.
...
│           ├─ change-review-merge.tsx    # Lazy CodeMirror editable merge dialog
...
`ui/change-review.tsx` opens from the Composer summary as a wide split pane or focus-trapped overlay drawer whose title bar, project sidebar, dividers, and covered conversation are inert. It provides the rounded run picker, filtered/aggregated directory tree with full arrow-key navigation, unified or Codex-style split diff, line wrapping, whitespace-only filtering, lightweight syntax highlighting, hunk navigation and resolution, incremental row folding, path copy, precise file/global truncation messages, and actionable non-Git/Git/storage/detail-load retry states. Complete UTF-8 text patches expose per-hunk accept/revert; revert requires confirmation and applies only an exact zero-fuzz reverse patch. `change-review-merge.tsx` lazily loads CodeMirror's unified merge view for per-chunk controls and arbitrary final edits. PiHost reconstructs the unresolved baseline, confines regular files to the project, serializes mutations, writes atomically, and rejects a save if the file revision changed after the editor opened. Resolution metadata is persisted in the bounded review sidecar. Review open/run/file/tree/split/options/scroll state is bounded and persisted per Session by `use-change-review.ts`; patches remain Host-owned. A newly queued empty Follow-up cannot hide the newest non-empty summary. The UI labels the result as interval worktree changes because concurrent external edits cannot be attributed exclusively to Pi. Staging and commit orchestration remain outside this mapping; binary, oversized, or truncated patches stay read-only.
...
`settings-command-handler.ts` reads Pi's effective non-TUI settings through Pi's getters and writes only changed retry/compaction/branch-summary (including `skipPrompt`)/network/image/trust/shell/session/resource/telemetry keys through `FileSettingsStorage.withLock`, preserving nested provider retry policy and unknown settings. Existing proxy credentials are stripped from the summary and preserved unless explicitly replaced. The session-tree overlay reads the effective `skipPrompt` value and hides its summary control when Pi's no-summary path is configured. Network, shell, and session-directory changes apply after a Host/application restart; default tools apply to newly created sessions.
...
`extension-theme.ts` in PiHost retains actual SDK Theme objects and resource-loaded themes. Pi's version-matched theme companion module is capability-guarded by `pi-adapter`. A stable proxy preserves `ui.theme` after SDK context copying. Theme events are `extension.ui.presentation` with `action: "theme"` and an `ExtensionThemeSnapshot`: only hex colors and a light/dark appearance cross IPC. Renderer validates a fixed token allowlist and applies it to the active Session's shell and overlays. Interactive `custom` factories are adapted into bounded terminal-style lines and key input; component instances are never serialized.
...
The Session Tree uses fixed-width branch rails rather than increasing indentation for each message. Host projection is iterative and capped at 5,000 entries with an explicit truncation notice; Renderer pages contain at most 160 rows. Skill invocations show the skill name and user request instead of expanding the embedded instructions. Info/warning notices remain for eight seconds, errors remain until dismissed, and every notice has a close button. Permission configuration repairs only a trailing literal `\n` after otherwise valid JSON; other parse/write failures do not overwrite the file or change the in-memory mode. Missing bash fallbacks are made explicit while existing command-specific rules and explicit fallbacks are preserved.
...
- Pi Session export/compact → session operations in Quick settings; Session Tree `/fork`, `/clone`, and `/tree` → one focus-trapped branch browser. PiHost projects `SessionManager.getTree()` into bounded serializable summaries, marks the current active path, and validates every selected entry. Fork uses `AgentSessionRuntime.fork()` and returns Pi's selected user text to the Composer; clone forks the current leaf at-position; tree navigation stays in the same Session and can optionally call Pi's abortable branch summarizer. Session replacement events move Renderer state to the new Pi Session identity before its timeline is loaded.
