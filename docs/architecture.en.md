...
`settings-command-handler.ts` reads Pi's effective non-TUI settings through Pi's getters and writes only changed retry/compaction/branch-summary/network/image/trust/shell/session/resource/telemetry keys through `FileSettingsStorage.withLock`, preserving nested provider retry policy and unknown settings. Existing proxy credentials are stripped from the summary and preserved unless explicitly replaced. Network, shell, and session-directory changes apply after a Host/application restart; default tools apply to newly created sessions.
