...
- Owning Pi `AgentSessionRuntime` so Extension commands receive RPC mode (with a scoped interactive-mode bridge for Pi's native `/llama` UI), official command-context actions, Session replacement/rebinding, diagnostics, async errors, shutdown requests, and cancellable/timeout-bound Extension UI requests.
