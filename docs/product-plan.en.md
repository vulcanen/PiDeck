...
`/skill:name` must go through `AgentSession.prompt()`; Pi SDK handles Skill expansion. PiDeck renders the expanded block as a collapsed Skill reference card with source and Markdown content, while keeping the optional user follow-up separate from the injected body.
