...
| API Key / OAuth | Provider settings (local credentials, removal confirmation) | `ModelRuntime.login()`, `ModelRuntime.logout()`, Pi auth callback; OpenAI Codex browser login preflights Pi 0.84.2–0.84.3's fixed loopback port, keeps manual callback entry as an explicit fallback, and refocuses the desktop window after success; PiHost initializes Pi's proxy-aware HTTP dispatcher with explicit environment → Pi `httpProxy` → Electron system-proxy precedence before token exchange |
