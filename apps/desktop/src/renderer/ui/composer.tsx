...
function Composer(props: ComposerProps) {
  ...
  return <div className="composer-wrap"><div className="composer-shell">
    ...
    </div><span className="composer-hint">{t.shiftEnter}</span><button className="send-button" disabled={props.queueMutationBusy || (props.queueEdit ? !hasDraft : !props.isSending && !hasDraft)} aria-label={props.queueEdit ? t.queueEditSave : stopOnly ? t.stop : t.send} title={props.queueEdit ? t.queueEditSave : stopOnly ? t.stop : t.send} onClick={stopOnly ? props.onStop : props.onSend}><Icon name={props.queueEdit ? "check" : stopOnly ? "stop" : "send"} size={16} /></button></div></div>
  </div></div>;
}
