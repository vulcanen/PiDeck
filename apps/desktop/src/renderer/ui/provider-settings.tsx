...
function ProviderSettings({ language, focusProviderId, onClose, onModelsRefresh }: { language: Language; focusProviderId: string | null; onClose: () => void; onModelsRefresh: (providerId?: string) => Promise<void> | void }) {
  ...
  function authErrorMessage(error: unknown): string {
    ...
    return message.includes("PIDECK_OAUTH_CALLBACK_UNAVAILABLE") ? t.oauthCallbackUnavailable : message;
  }
  async function resolvePrompt(value: string) {
    ...
    catch (error) { setAuthError(authErrorMessage(error)); return; }
    setAuthPrompt(null);
  ...
  }, !authPromptVisible && !pendingLogout);
  ...
  useEffect(() => window.pideck.events.subscribe((runtimeEvent) => {
    ...
    if (event?.type === "prompt") {
      ...
      else { setAuthNotice(null); setAuthUrl(null); }
      ...
      setAuthUrl(null); setAuthNotice(t.oauthWaiting); setManualAuthVisible(false);
      if (event.event.url) {
      ...
      setAuthUrl(null); setAuthNotice(event.event.userCode ? t.authDeviceCode(String(event.event.userCode)) : t.oauthWaiting); setManualAuthVisible(false);
  ...
  return <div className="settings-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !busyProvider) onClose(); }}><section ref={dialogRef} className="settings-sheet" role="dialog" aria-modal="true" aria-labelledby="provider-title"><div className="settings-header"><div><span className="eyebrow">{t.localRuntime}</span><h2 id="provider-title">{t.providerAuthTitle}</h2></div><button className="icon-button" onClick={onClose} aria-label={t.closeSettings} title={t.closeSettings}><Icon name="x" /></button></div><div className="locality-note"><span className="status-dot" /><span>{t.localCredentials}</span></div>{!authPromptVisible && authNotice && <div className="auth-notice" role="status"><div>{authNotice}</div><div className="auth-notice-actions">{authUrl && <button className="button primary" onClick={() => void window.pideck.providers.openAuthUrl(authUrl)}>{t.openBrowser}</button>}{authPrompt?.type === "manual_code" && <button className="button ghost" onClick={() => setManualAuthVisible(true)}>{t.enterAuthCodeManually}</button>}</div></div>}{!authPromptVisible && authError && <div className="auth-error" role="alert"><div className="auth-error-copy">{authError}</div><div className="auth-error-actions">{authUrl && <button className="button primary" onClick={() => void window.pideck.providers.openAuthUrl(authUrl)}>{t.openBrowser}</button>}<button className="button ghost" onClick={() => { setAuthError(null); void loadProviders(); }}>{t.retry}</button></div></div>}
    {!loading && !listError && providers.length > 0 && <div className="provider-tools"><label className="provider-search"><Icon name="search" size={14} /><input type="search" value={providerQuery} onChange={(event) => setProviderQuery(event.target.value)} placeholder={t.providerSearch} aria-label={t.providerSearch} /></label><div className="provider-filters" role="group" aria-label={t.providerAuthTitle}>{(["all", "configured", "expired", "missing"] as ProviderFilter[]).map((filter) => <button type="button" key={filter} aria-pressed={providerFilter === filter} onClick={() => setProviderFilter(filter)}>{filter === "all" ? t.providerFilterAll : filter === "configured" ? t.providerFilterConfigured : filter === "expired" ? t.providerFilterExpired : t.providerFilterMissing}</button>)}</div></div>}
